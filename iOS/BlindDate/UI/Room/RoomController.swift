...
class RoomController: BaseViewContoller, DialogDelegate, RoomDelegate {
    @IBOutlet var backView: UIButton!
    @IBOutlet var showMembersView: UIButton!
    @IBOutlet var avatar0: RoundImageView!
    @IBOutlet var avatar1: RoundImageView!
    @IBOutlet var avatar2: RoundImageView!

    @IBOutlet var hostRootView: UIView!
    @IBOutlet var hostVideoView: UIView! {
        didSet {
            hosterVideoView.videoView = hostVideoView
        }
    }

    @IBOutlet var hostNameView: UILabel! {
        didSet {
            hosterVideoView.nameView = hostNameView
        }
    }

    @IBOutlet var hostMicView: UIImageView! {
        didSet {
            hosterVideoView.micView = hostMicView
        }
    }

    @IBOutlet var leftRootView: UIView!
    ...
    @IBOutlet var leftNameView: UILabel! {
        didSet {
            leftSpeakerVideoView.nameView = leftNameView
        }
    }

    @IBOutlet var leftMicView: UIImageView! {
        didSet {
            leftSpeakerVideoView.micView = leftMicView
        }
    }

    @IBOutlet var leftAddView: UIButton! {
        didSet {
            leftSpeakerVideoView.addView = leftAddView
        }
    }

    @IBOutlet var rightRootView: UIView!
    ...
    @IBOutlet var rightNameView: UILabel! {
        didSet {
            rightSpeakerVideoView.nameView = rightNameView
        }
    }

    @IBOutlet var rightMicView: UIImageView! {
        didSet {
            rightSpeakerVideoView.micView = rightMicView
        }
    }

    @IBOutlet var rightAddView: UIButton! {
        didSet {
    ...
    @IBOutlet var toolBarRootView: UIView!
    @IBOutlet var inputMessageRootView: UIView!
    @IBOutlet var inputMessageView: UITextField!
    private var showInputView: Bool = false

    private var hosterVideoView = RoleVideoView()
    private var leftSpeakerVideoView = RoleVideoView()
    private var rightSpeakerVideoView = RoleVideoView()
    ...
    private func onMemberEnter(member: BlindDateMember) {
        ...
        view.marginLeading(anchor: self.view.leadingAnchor, constant: 12)
            .marginTop(anchor: chatListView.topAnchor)
            .width(constant: 257)
...
extension RoomController: RoomControlDelegate {
    func onTap(view: RoleVideoView) {
        if viewModel.isManager {
            ...
            if let left = left {
                showAlert(title: "申请连麦", message: "红娘同意后会自动开始视频连麦")
                    .filter { ok in
                        ok
                    }
                    .flatMap { [unowned self] _ -> Observable<Result<Void>> in
                        self.viewModel.handsup(left: left)
                    }
                    .subscribe(onNext: { [unowned self] result in
                        if !result.success {
...
extension RoomController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.message = viewModel.messageList[viewModel.messageList.count - 1 - indexPath.row]
        return cell
    }
}
