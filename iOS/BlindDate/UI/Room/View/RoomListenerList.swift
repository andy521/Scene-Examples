...
extension RoomListenerList: UITableViewDataSource {
    func tableView(_: UITableView, numberOfRowsInSection _: Int) -> Int {
        return delegate.viewModel.listeners.list.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.isManager = delegate.viewModel.isManager
        cell.item = delegate.viewModel.listeners.list[indexPath.row]
        cell.delegate = self
        return cell
    }
}
...
extension RoomListenerList: ListenerListDelegate {
    func invate(member: BlindDateMember) -> Observable<Result<Void>> {
        return delegate.viewModel.inviteSpeaker(member: member)
            .flatMap { result -> Observable<Result<Void>> in
                result.onSuccess { () -> Observable<Result<Void>> in
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        inviteButton.rx.tap
            .flatMap { [unowned self] _ in
                self.delegate.invate(member: self.item)
            }
            .subscribe()
            .disposed(by: disposeBag)
    }
