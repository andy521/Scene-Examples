...
class MainVM: NSObject {
    ...
    var videoConfig: AgoraVideoEncoderConfiguration {
        ...
        return AgoraVideoEncoderConfiguration(size: size,
                                              frameRate: .fps15,
                                              bitrate: 700,
    ...
    func addRoomInfo() {
        ...
        let property = config.entryType == .asCreator ? info.dict : [:]
        sceneRef.update(data: property) { [weak self](_) in
            Log.info(text: "addRoomInfo success",
                     tag: "MainVM")
            self?.addMember()
        } fail: { [weak self](error) in
            Log.info(text: "addRoomInfo fail: \(error.errorDescription ?? "")",
                     tag: "MainVM")
            guard let self = self else { return }
            self.invokeMainVM(self, shouldShow: error.errorDescription ?? "")
        }
    ...
    func addMember() { /** 把本地用户添加到人员列表 **/
        ...
        sceneRef.collection(className: "member")
            .add(data: userInfo.dict) { [weak self](_) in
                ...
                self.config.entryType == .asCreator ? self.joinRtcByPush() : self.initMediaPlayer(config: self.config)
