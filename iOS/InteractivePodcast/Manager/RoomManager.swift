...
extension RoomManager: IRoomManager {
            ...
            if rtcServer.channel == room.id {
                ...
                return Observable.just(rtcServer.isJoinChannel)
                    ...
                    .concatMap { result -> Observable<Result<PodcastRoom>> in
                        result.onSuccess { PodcastRoom.getRoom(by: room.id) }
                    }
                    .concatMap { result -> Observable<Result<Void>> in
    ...
    func leave() -> Observable<Result<Void>> {
        if let member = member {
            if rtcServer.isJoinChannel {
                return Observable.zip(rtcServer.leaveChannel(), member.leave()).map { result0, result1 in
    ...
    func subscribeMembers() -> Observable<Result<[PodcastMember]>> {
        ...
        .map { [unowned self] args -> Result<[PodcastMember]> in
