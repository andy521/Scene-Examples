...
class MiniRoomView: Dialog {
    ...
    private var icon: UIImageView = {
        ...
        view.image = UIImage(named: "blueBroadcaster", in: Utils.bundle, with: nil)
    ...
    private var managerToolbar: ManagerMiniToolbar?
    private var speakerMiniToolbar: SpeakerMiniToolbar?
    private var listenerMiniToolbar: ListenerMiniToolbar?
    ...
    private func renderAvatars(speakers: Int) -> UIView? {
        ...
        for index in 1 ... count {
            ...
            if _index < users.count {
                avatar.image = UIImage(named: users[_index].getLocalAvatar(), in: Utils.bundle, with: nil)
            } else {
                avatar.image = nil
            }
    ...
    override func render() {
        rounded()
        shadow()
        let avatar = renderAvatars(speakers: max(viewModel.speakersCount, 1))
        addSubview(label)
    ...
    func show(with delegate: HomeController, onDismiss: (() -> Void)? = nil) {
        ...
        show(controller: delegate, style: .bottomNoMask, padding: 16, onDismiss: onDismiss)
    }
    ...
    func show(message: String, type: NotificationType, duration: CGFloat = 1.5) {
        delegate.show(message: message, type: type, duration: duration)
    }
