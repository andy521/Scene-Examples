...
public class PodcastMember {
    ...
    public init(id: String, isMuted: Bool, isSelfMuted: Bool, isSpeaker: Bool, room: PodcastRoom, streamId: UInt, user: User) {
        ...
        self.user = user
        isManager = room.anchor.id == user.id
    }

    public func action(with action: PodcastActionType) -> PodcastAction {
        return PodcastAction(id: "", action: action, status: .ing, member: self, room: room)
    }
}
...
extension PodcastRoom {
    ...
    static func getRooms() -> Observable<Result<[PodcastRoom]>> {
    ...
    func getMembers() -> Observable<Result<[PodcastMember]>> {
        return PodcastRoom.manager.getMembers(room: self)
    }

    func getCoverSpeakers() -> Observable<Result<[PodcastMember]>> {
        return PodcastRoom.manager.getCoverSpeakers(room: self)
    }

    func subscribeMembers() -> Observable<Result<[PodcastMember]>> {
