    ...
    class FirebasePodcastModelManager: IPodcastModelManager {
        ...
        func getAction(objectId: String) -> Observable<Result<PodcastAction>> {
            return Database.query(className: PodcastAction.TABLE, objectId: objectId) { (object: DocumentSnapshot) -> PodcastAction in
                ...
                result.onSuccess { () -> Observable<Result<PodcastAction>> in
        ...
        func getMember(by objectId: String) -> Observable<Result<PodcastMember>> {
            return Database.query(className: PodcastMember.TABLE, objectId: objectId) { (object: DocumentSnapshot) -> PodcastMember in
                ...
                result.onSuccess { () -> Observable<Result<PodcastMember>> in
        ...
        func create(room: PodcastRoom) -> Observable<Result<String>> {
            return Database.save { () -> (String, data: [String: Any], String?) in
                (PodcastRoom.TABLE, [PodcastRoom.CHANNEL_NAME: room.channelName, PodcastRoom.ANCHOR_ID: Database.document(table: User.TABLE, id: room.anchor.id)], nil)
            }
        }
        ...
        func getRooms() -> Observable<Result<[PodcastRoom]>> {
            return Database.query(className: PodcastRoom.TABLE) { (ref: CollectionReference) -> Query in
                ...
                data.map { (object: DocumentSnapshot) -> String in
                    object.documentID
                }
            }.flatMap { (result: Result<[String]>) -> Observable<Result<[PodcastRoom]>> in
                result.onSuccess { () -> Observable<Result<[PodcastRoom]>> in
                    ...
                    if roomIds.count == 0 {
                        ...
                        return Observable.zip(roomIds.map { (roomId: String) -> Observable<Result<PodcastRoom>> in
                            self.getRoom(by: roomId)
                        }).map { (results: [Result<PodcastRoom>]) -> Result<[PodcastRoom]> in
        ...
        func update(room: PodcastRoom) -> Observable<Result<String>> {
            return Database.save { () -> (String, data: [String: Any], String?) in
                (PodcastRoom.TABLE, [PodcastRoom.CHANNEL_NAME: room.channelName, PodcastRoom.ANCHOR_ID: Database.document(table: User.TABLE, id: room.anchor.id)], room.id)
            }
        }
        ...
        func getMembers(room: PodcastRoom) -> Observable<Result<[PodcastMember]>> {
            ...
            }.flatMap { (result: Result<[String]>) -> Observable<Result<[PodcastMember]>> in
                result.onSuccess { () -> Observable<Result<[PodcastMember]>> in
                    ...
                    if memberIds.count == 0 {
                        ...
                        return Observable.zip(memberIds.map { (memberId: String) -> Observable<Result<PodcastMember>> in
                            self.getMember(by: memberId)
                        }).map { (results: [Result<PodcastMember>]) -> Result<[PodcastMember]> in
        ...
        func subscribeMembers(room: PodcastRoom) -> Observable<Result<[PodcastMember]>> {
            ...
            .flatMap { result -> Observable<Result<[PodcastMember]>> in
                result.onSuccess { self.getMembers(room: room) }
            }
        }
        ...
        func handsup(member: PodcastMember) -> Observable<Result<Void>> {
            ...
            return Database.save { () -> (String, data: [String: Any], String?) in
                FirebasePodcastModelManager.toData(action: action)
            }
            .map { $0.transform() }
        }

        func inviteSpeaker(master _: PodcastMember, member: PodcastMember) -> Observable<Result<Void>> {
