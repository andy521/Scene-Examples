...
//
    ...
    class LeanCloudBlindDateModelManager: IBlindDateModelManager {
        ...
        func getRooms() -> Observable<Result<[BlindDateRoom]>> {
            ...
            } transform: { list -> [BlindDateRoom] in
                let rooms: [BlindDateRoom] = try list.map { room in
                    try LeanCloudBlindDateModelManager.from(object: room)
                }
                return rooms
            }
        }
        ...
        func getMembers(room: BlindDateRoom) -> Observable<Result<[BlindDateMember]>> {
            ...
            } transform: { list -> [BlindDateMember] in
                let members: [BlindDateMember] = try list.map { data in
        ...
        func subscribeMembers(room: BlindDateRoom) -> Observable<Result<[BlindDateMember]>> {
            ...
            .flatMap { [unowned self] result -> Observable<Result<[BlindDateMember]>> in
...
//                Logger.log(message: object.jsonString, level: .info)
            ...
            .concatMap { result in
                LeanCloudBlindDateModelManager.getBlindDateAction(objectId: result.data!)
            }
        }
