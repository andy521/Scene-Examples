...
//
    ...
    class LeanCloudPodcastModelManager: IPodcastModelManager {
        ...
        func getRooms() -> Observable<Result<[PodcastRoom]>> {
            ...
            } transform: { list -> [PodcastRoom] in
                let rooms: [PodcastRoom] = try list.map { room in
                    try LeanCloudPodcastModelManager.from(object: room)
                }
                return rooms
            }
        }
        ...
        func getMembers(room: PodcastRoom) -> Observable<Result<[PodcastMember]>> {
            ...
            } transform: { list -> [PodcastMember] in
                let members: [PodcastMember] = try list.map { data in
        ...
        func subscribeMembers(room: PodcastRoom) -> Observable<Result<[PodcastMember]>> {
            ...
            .flatMap { [unowned self] result -> Observable<Result<[PodcastMember]>> in
...
//                Logger.log(message: object.jsonString, level: .info)
            ...
            .concatMap { result in
                LeanCloudPodcastModelManager.getPodcastAction(objectId: result.data!)
            }
        }
