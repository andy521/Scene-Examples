...
//
    ...
    class AgoraObject: IAgoraObject {
        ...
        func getId() throws -> String {
            Logger.log(message: "AgoraObject getId", level: .info)
            return object.objectId!.stringValue!
        }

        func getValue(key: String, type: Any.Type) throws -> Any? {
            Logger.log(message: "AgoraObject getValue \(key)", level: .info)
            if type == String.self {
        ...
        func toObject<T>() throws -> T where T: Decodable {
            Logger.log(message: "AgoraObject toObject \(object.jsonValue)", level: .info)
            let data: Data
