...
extension LiveKtvMember {
    ...
    static func get(object: IAgoraObject, room: LiveKtvRoom) throws -> LiveKtvMember {
        ...
        // let room: AgoraDocumentReference = try object.getValue(key: LiveKtvMember.ROOM, type: AgoraDocumentReference.self) as! AgoraDocumentReference
    ...
    func leave() -> Observable<Result<Void>> {
        if isManager {
            return Observable.zip(
                ...
                LiveKtvAction.delete(roomId: roomId)
            ).map { args in
    ...
    func orderMusic(name: String, music: String) -> Observable<Result<LiveKtvMusic>> {
        let music = LiveKtvMusic(id: "", memberId: id, roomId: roomId, name: name, musicId: music)
        return music.order()
    }
}
